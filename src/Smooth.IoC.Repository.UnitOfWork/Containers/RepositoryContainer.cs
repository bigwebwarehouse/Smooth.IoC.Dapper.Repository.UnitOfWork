...
internal sealed class RepositoryContainer
{
    ...
    private readonly ConcurrentDictionary<Type, PropertyMapping[]> _keys = new();
    private readonly ConcurrentDictionary<Type, IEnumerable<PropertyInfo>> _properties = new();
    private readonly ConcurrentDictionary<Type, bool> _isIEntity = new();
    ...
    {
        return _isIEntity.GetOrAdd(typeof(TEntity),
            typeof(TEntity).GetInterfaces().Any(x => x == typeof(IEntity<TPk>)));
    }
    ...
    {
        return entity.GetProperties().Where(property =>
            keys.Any(key => property.Name.Equals(key.PropertyName, StringComparison.Ordinal)));
    }
}